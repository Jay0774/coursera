...
if ( isset($_GET['code']) ) {
    $code = $_GET['code'];
    if ( strlen($code) != 4 ) {
        $error = "Input must be exactly 4 digits";
    } else if ( $code[0] < 0 || $code[0] > 9 || 
                $code[1] < 0 || $code[1] > 9 || 
                $code[2] < 0 || $code[2] > 9 || 
                $code[3] < 0 || $code[3] > 9 ) {
        $error = "Input must 4 digit pin";
    } else {
        $md5 = hash('md5', $code);
    }
}
?>
<!DOCTYPE html>
<head><title>Jayesh Budhwani PIN Code</title></head>
<body>
<h1>MD5 PIN Maker</h1>
<?php
if ( $error !== false ) {
...
?>
<p>Please enter a 4-digit pin for encoding.</p>
<form>
